...
Currently, [VS Code - Remote SSH](https://code.visualstudio.com/docs/remote/ssh) does not automatically install local user extensions on remote machines. Gitpod extension prompts users to enable [VS Code Setting Sync](/docs/ides-and-editors/settings-sync#enabling-settings-sync-in-vs-code-desktop) with Gitpod to retrieve local extensions and automatically install extensions in your Gitpod workspace. You can enable, or disable syncing of local extensions via the `gitpod.remote.syncExtension` VS Code settings. If you have feedback, please add it to the GitHub issue: [#7906](https://github.com/gitpod-io/gitpod/issues/7906).
